...
int proc_init(void) {
	...
	// printk( KERN_INFO "GOLDEN_RATIO_PRIME is: %lu\n", GOLDEN_RATIO_PRIME);
        ...
        start = jiffies / HZ; // conversion for seconds - start time
	return 0;
}
...
void proc_exit(void) {
        ...
        printk( KERN_INFO "In proc_exit(), jiffies is: %lu and HZ is: %d\n", jiffies, HZ);
        //  end = jiffies / HZ; // conversion for seconds - end time
        //  elapsed = end - start; 
        printk( KERN_INFO "The elapsed time is: %lu seconds.\n", elapsed);
...
ssize_t proc_read(struct file *file, char __user *usr_buf, size_t count, loff_t *pos) {
        ...
        rv = sprintf(buffer, "The elapsed time is: %lu seconds.\n", elapsed);
